...
class MOEAD(object):
    ...
    def __init__(self, population, toolbox, mu, cxpb, mutpb, ngen=0, maxEvaluations=0,
        ...
        if population:
            ...
            fitnesses = self.toolbox.map(
                self.toolbox.evaluate, self.population)
    ...
    def execute(self):
        ...
        self.indArray_ = [self.toolbox.individual()
        ...
        record = self.stats.compile(
            self.population) if self.stats is not None else {}
        ...
        while self.evaluations_ < self.maxEvaluations:
            ...
            for i in range(self.populationSize_):
                ...
                logbook.record(
                    gen=self.ngen, evals=self.evaluations_, **record)
    ...
    def initUniformWeight(self):
        ...
        if self.n_objectives == 2:
            ...
            self.lambda_ = sorted((x for x in self.lambda_),
                                  key=lambda x: sum(x), reverse=True)
            self.lambda_ = self.lambda_[:self.populationSize_]
        else:
            dataFileName = "W" + str(self.n_objectives) + \
                "D_" + str(self.populationSize_) + ".dat"
                ...
                with open(file_, 'r') as f:
                    ...
                    for aux_ in f:
                        ...
                        try:
                            ...
                            print("tokens", tokens)
                            print("value:",  value)
                            print("lambda_", len(self.lambda_),
                                  "*", len(self.lambda_[0]))
                            print(i, j, aux_)
                            print(e)
                            raise e
    ...
    def initIdealPoint(self):
        for i in range(self.n_objectives):
            ...
            self.indArray_[i].fitness.values = self.toolbox.evaluate(self.indArray_[
                                                                     i])
            # mbelmadani: For minimization objectives
            self.z_[i] = 1e30 * self.indArray_[i].fitness.weights[i]
            self.evaluations_ += 1
    ...
    def fitnessFunction(self, individual, lambda_):
        ...
        if self.functionType_ == "_TCHE1":
            ...
            for n in range(self.n_objectives):
                # JMetal default
                diff = abs(individual.fitness.values[n] - self.z_[n])
                feval = float()
                if lambda_[n] == 0:
                    feval = 0.0001 * diff
                else:
